  ...
  return (
    <div className="w-full max-w-[820px] mx-auto min-h-screen pb-10">
      ...
      <header className="bg-[#1B3A3D] text-white rounded-b-[28px] px-6 pt-12 pb-10 text-center shadow-sm">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-white/10 text-white mb-3 text-4xl shadow-inner">
          👋
        </div>
        ...
        <div className="bg-white border-[1.5px] border-[#D9CFB4] rounded-2xl p-6 md:p-8 text-center shadow-sm space-y-6">
          ...
          <div className="space-y-3 pt-2">
            <button
              onClick={onEnter}
              id="welcomeEnterBtn"
              className="w-full flex items-center justify-center gap-2 bg-[#1B3A3D] hover:bg-[#152e30] active:scale-[0.98] transition-transform text-white font-bold py-3.5 px-6 rounded-xl text-lg shadow-sm cursor-pointer"
            >
            ...
            <button
              onClick={onLogout}
              id="welcomeLogoutBtn"
              className="text-[#5C7376] hover:text-[#A8432E] text-sm underline py-2 transition-colors block mx-auto cursor-pointer"
            >
              تسجيل الخروج
            </button>
          </div>
